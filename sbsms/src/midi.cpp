...
void processMIDIEvents(VstEvents *events, BlockEvents *blockEvents, int *numBlockEvents)
{
  ...
  for (i = 0; (i < events->numEvents); i++) {
    ...
    if ( (status == 0x90) || (status == 0x80) ) {
      ...
      blockEvents[*numBlockEvents].eventStatus = midiNote;	// status
    ...
    // looking at pitchbend   (0xe* is Pitchbend status)
      ...
      blockEvents[*numBlockEvents].eventStatus = midiPitchbend;	// status
      ...
      blockEvents[*numBlockEvents].eventStatus = midiChannelAfterTouch;	// status
      blockEvents[*numBlockEvents].byte1 = midiData[1] & 0x7f;	// LSB
      blockEvents[*numBlockEvents].byte2 = 0;
      blockEvents[*numBlockEvents].delta = midiEvent->deltaFrames;	// timing offset
      ...
      blockEvents[*numBlockEvents].eventStatus = midiPolyphonicAfterTouch;	// status
      blockEvents[*numBlockEvents].byte1 = midiData[1] & 0x7f;	// LSB
      blockEvents[*numBlockEvents].byte2 = midiData[2] & 0x7f;	// MSB
      blockEvents[*numBlockEvents].delta = midiEvent->deltaFrames;	// timing offset
    ...
    // all notes off
      ...
      blockEvents[*numBlockEvents].eventStatus = midiNotesOff;	// status
      blockEvents[*numBlockEvents].delta = midiEvent->deltaFrames;	// timing offset
      
      (*numBlockEvents)++;
    } else if ( (status == 0xb0) ) {
      blockEvents[*numBlockEvents].eventStatus = midiControl;	// status
      blockEvents[*numBlockEvents].byte1 = midiData[1] & 0x7f;	// LSB
      blockEvents[*numBlockEvents].byte2 = midiData[2] & 0x7f;	// MSB
      blockEvents[*numBlockEvents].delta = midiEvent->deltaFrames;	// timing offset
...
out any parts of it, let me know & I'll explain.
	There is one more floatly important thing in process().  At the of the 
