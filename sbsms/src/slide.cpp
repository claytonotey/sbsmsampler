...
public:
  LinearInputRateSlide(float rate0, float rate1, countType n) {
    this->rate0 = rate0;
    this->rate1 = rate1;
    if(n) {
      val = rate0;
...
public:
  LinearOutputRateSlide(float rate0, float rate1, countType n) {
    this->rate0 = rate0;
    this->rate1 = rate1;
    if(n) {
      val = 0.0;
...
public:
  LinearInputStretchSlide(float rate0, float rate1, countType n) {
    this->rate0 = rate0;
    this->rate1 = rate1;
    if(n) {
      val = 1.0 / rate0;
...
public:
  LinearOutputStretchSlide(float rate0, float rate1, countType n) {
    this->rate0 = rate0;
    this->rate1 = rate1;
    c0 = rate0 / rate1;
...
public:
  GeometricInputSlide(float rate0, float rate1, countType n) {
    this->rate0 = rate0;
    this->rate1 = rate1;
    if(n) {
      val = rate0;
...
public:
  GeometricOutputSlide(float rate0, float rate1, countType n) {
    this->rate0 = rate0;
    this->rate1 = rate1;
...
Slide :: Slide(SlideType slideType, float rate0, float rate1, countType n) 
{
  assert(rate0 > 0.0f);
  assert(rate1 > 0.0f);
                                                     ...
                                                     SBSMSQuality *quality)
{
  ...
  this->samplesToOutput = (countType)((float)samplesToInput * totalStretch);
  stretchScale = 1.0f;

  if(quality) {
    countType samplesIn = 0;
    countType samplesOut = 0;
    ...
    countType samplesOutputed = samplesOut - lrintf(stretch * (samplesIn - samplesToInput));
...
countType SBSMSInterfaceSliding :: getSamplesToInput() { return imp->getSamplesToInput(); }
countType SBSMSInterfaceSlidingImp :: getSamplesToInput()
{
  return samplesToInput;
}

countType SBSMSInterfaceSliding :: getSamplesToOutput() { return imp->getSamplesToOutput(); }
countType SBSMSInterfaceSlidingImp :: getSamplesToOutput()
{
  return samplesToOutput;
}

SBSMSInterfaceSliding :: SBSMSInterfaceSliding(Slide *stretchSlide, Slide *pitchSlide, 
                                               bool bPitchReferenceInput,
